...
### 로봇에 달려있는 라이다와 카메라 간의 위치 및 자세 정보를 받은 다음, 이를 가지고 좌표 변환 행렬을 만들고, 카메라 이미지에 라이다 포인트들을 projection
하는 노드

### 아래 코드에서 이미지에 정사영시 라이다 포인트들이 왼쪽으로 치우치는 현상발견.. (계속 수정될 예정)
...
def transformMTX_lidar2cam(params_lidar, params_cam):
    ...
    Rmtx = rotationMtx(math.pi * (90 / 180), 0.0, math.pi * (90 / 180))
    # Rmtx = rotationMtx(math.pi * (180 / 180), math.pi * (-90 / 180), 0)
    # Rmtx = np.matmul(rotationMtx(math.pi * (-90 / 180), 0.0, 0), rotationMtx(0, 0.0, math.pi * (180 / 180)))
    

    """
    로직 4. 위의 두 행렬을 가지고 최종 라이다-카메라 변환 행렬을 정의
    """
    # RT = np.matmul(Tmtx, Rmtx) 
...
def project2img_mtx(params_cam):
    ...
    fc_x = params_cam['HEIGHT'] / (2 * math.tan( math.pi * (params_cam['FOV'] / 2) / 180))
    fc_y = params_cam['HEIGHT'] / (2 * math.tan( math.pi * (params_cam['FOV'] / 2) / 180))
...
class LIDAR2CAMTransform:
    ...
    def transform_lidar2cam(self, xyz_p):
        ...
        """
        # R_RT = np.linalg.inv(self.RT)
        # xyz_c = [np.matmul(R_RT, np.c_[np.append(xyz, [1],axis=0)]) for xyz in xyz_p]
        xyz = xyz_p[0]
        xyz_c = np.matmul(self.RT, np.c_[np.append(xyz, [1],axis=0)]).T
        for i in range(1, len(xyz_p)):
            xyz_c = np.concatenate((xyz_c, np.matmul(self.RT, np.c_[np.append(xyz_p[i], [1],axis=0)]).T),axis=0)
    ...
    def project_pts2img(self, xyz_c, crop=True):
        ...
        xn, yn, zn = xyz_c[:,0], xyz_c[:,1], xyz_c[:,2]
        xn, yn = xn / zn, yn / zn
    ...
    def crop_pts(self, xyi):
        ...
        # np.logical_and -> 안의 두조건을 만족하는 값만 출력
...
class SensorCalib(Node):
    ...
    def scan_callback(self, msg):
        ...
        #     print(angle, R)
        # self.angle, self.R = enumerate(msg.ranges) 
        # print(self.angle, self.R)
        self.R = msg.ranges

        x = np.array([ R * math.cos(math.pi * angle / 180)for angle, R in enumerate(self.R)])
        y = np.array([ R * math.sin(math.pi * angle / 180)for angle, R in enumerate(self.R)])
        z = np.zeros(shape=(360,), dtype=np.int8)
    ...
    def timer_callback(self):
        ...
        if self.xyz is not None and self.img is not None :
            ...
            img_l2c = draw_pts_img(self.img, list(map(int,xy_i[:,0])), list(map(int,xy_i[:,1])))
